...
```{r}
sapply(list(Train=cmx, Test=cmx2), A2x2) #過度配適使train和test相差大
```
<br><br><hr>
...
```{r fig.height=3.2, fig.width=7}
glm1 = glm(PoorCare ~ ., TR[,-1], family=binomial)
pred = predict(glm1, type="response")
pred2 = predict(glm1, newdata=TS, type="response")
...
```{r}
#考量整體模型下，AUC越高預測越準確，這一個模型比較準。
```
&emsp; 【F】 為什麼它比較準(或比較不準)呢？<br><br>
```{r}
#因為第二個模型有使用比較多顯著影響PoorCare的變數，解釋力比較高，所以預測的比較準。
```
</p class="qiz">
